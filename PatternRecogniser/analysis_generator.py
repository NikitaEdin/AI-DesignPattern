...
class AnalysisGenerator:
    ...
    def get_excel_reports(self, llm_name: str | None = None) -> list[Path]:
        """ Get Excel report files from the reports directory, optionally scoped to a single LLM """
        if not os.path.exists(self.reports_dir):
            raise FileNotFoundError(f"Reports directory not found: {self.reports_dir}")

        pattern = f"report_{llm_name}.xlsx" if llm_name else "report_*.xlsx"
        excel_files = list(Path(self.reports_dir).glob(pattern))

        if not excel_files:
            raise FileNotFoundError(f"No Excel reports found in {self.reports_dir} matching '{pattern}'")
    ...
    def run(self, llm_name: str | None = None) -> list[str]:
        """ process each LLM report separately, optionally scoped to a single LLM """
        print("="*60)
        print("Starting Analysis Generation")
        print("="*60)

        # Get Excel report(s)
        excel_files = self.get_excel_reports(llm_name)
        output_files = []
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate analysis JSON from Excel reports")
    parser.add_argument('--llm', type=str, default=None,
                         help="Only process this LLM's report (e.g. 'jev'), leaving other providers' analysis JSON untouched. Omit to process all reports.")
    ...
    # Run the analysis (scoped to a single LLM if --llm is given)
    try:
        output_files = generator.run(llm_name=args.llm)
